...
int main() {
...

  {
    ...
    print_value_n.operator()(union_state);
    print_value_n.operator()<true>(union_state);
    print_value_n.operator()<false, true>(union_state);
    print_value_n.operator()<true, true>(union_state);
  }
  cout << '\n';
  { // 0|1
    UnionState union_state{SymbolState{0}, SymbolState{1}};
